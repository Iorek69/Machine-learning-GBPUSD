...
def patternRecognition():
    ...
    for eachPattern in patternAr:
        ...
        if howSim > 70:
            patdex = patternAr.index(eachPattern) # what number in the pattern is it

            patFound = 1 #a pattern has been found
            
            '''print ('-----------------Recorded Patterns-----------------')
            print (patForRec)
            print ('-----------------Pattern-----------------')
            print (eachPattern)
    ...
    if patFound == 1:
        ...
        realAverageOutcome= functools.reduce(lambda x, y: x+y, realOutcomeRange) / len(realOutcomeRange)
        realMovement = percentChange(allData[toWhat],realAverageOutcome)
        predictedAverageOutcome = functools.reduce(lambda x, y: x+y, predictedOutcomeAr) / len(predictedOutcomeAr)
        ...
        plt.scatter(40, predictedAverageOutcome, c='b', s=25)
...
dataLength = int(bid.shape[0])#where should the while loop stop
print ('data lenth is',dataLength)

toWhat = 37000
allData = ((bid+ask)/2)

accuracyArray = [] #creates empty array
samps = 0
...
while toWhat < dataLength:
    ...
    patternAr =[]
    performanceAr = [] # this represents the future performance
    patForRec = [] # the pattern to recognise
    ...
    samps += 1 # keeps count of the current number of backtested data
    toWhat +=1 # up to what will the data test; 37,000 + 1 for example
    accuracyAverage = functools.reduce(lambda x, y: x+y, accuracyArray) / len(accuracyArray)
    print ('Backtested accuarcy is: ', str(accuracyAverage)+'% after', samps, 'samples')
